...
const { port, accessKey, secretKey, region, userPoolId, appClientId } = require("../../config");
AWS.config.update({
    region,
...
const jsonWebKeys = [
    // from https://cognito-idp.<REGION>.amazonaws.com/<UserPoolId>/.well-known/jwks.json
    // include the json web keys from GITLAB ----------------------- (this is a reference for the author)
    {
        alg: process.env.COGNITO_JSON_WEB_KEYS_ALG,
        e: process.env.COGNITO_JSON_WEB_KEYS_E,
        kid: process.env.COGNITO_JSON_WEB_KEYS_KID1,
        kty: process.env.COGNITO_JSON_WEB_KEYS_KTY,
        n: process.env.COGNITO_JSON_WEB_KEYS_N1,
        use: process.env.COGNITO_JSON_WEB_KEYS_USE,
    },
    {
        alg: process.env.COGNITO_JSON_WEB_KEYS_ALG,
        e: process.env.COGNITO_JSON_WEB_KEYS_E,
        kid: process.env.COGNITO_JSON_WEB_KEYS_KID2,
        kty: process.env.COGNITO_JSON_WEB_KEYS_KTY,
        n: process.env.COGNITO_JSON_WEB_KEYS_N2,
        use: process.env.COGNITO_JSON_WEB_KEYS_USE,
    },
];
        ...
        if (err.message === "jwt expired" || err.toString().split(":")[0] === "TokenExpiredError") {
            ...
            try {
                ...
                return res.status(500).send({
                    message: error.message || "Error validating tokens. Please sign in again.",
                });
            }
        } else {
            return res
                .status(500)
                .send({ message: err.message || "Error validating tokens. Please sign in again." });
        }
    }
};
